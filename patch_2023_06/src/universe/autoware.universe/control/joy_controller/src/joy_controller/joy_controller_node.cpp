...
#include "joy_controller/joy_converter/g923_joy_converter.hpp"
...
void AutowareJoyControllerNode::onJoy(const sensor_msgs::msg::Joy::ConstSharedPtr msg)
{
  ...
  } else if (joy_type_ == "G923") {
    joy_ = std::make_shared<const G923JoyConverter>(*msg);
...
{
  //NAREN - Change the gatemode as well as select the operational mode
...
: Node("joy_controller", node_options)
{
  ...
  //NAREN - To enable REMOTE mode from Joystick
  client_change_to_stop_ =
    this->create_client<ChangeOperationMode>("/api/operation_mode/change_to_stop");
  client_change_to_remote_ =
    this->create_client<ChangeOperationMode>("/api/operation_mode/change_to_remote");
