...
#ifndef JOY_CONTROLLER__JOY_CONVERTER__G923_JOY_CONVERTER_HPP_
#define JOY_CONTROLLER__JOY_CONVERTER__G923_JOY_CONVERTER_HPP_

#include "joy_controller/joy_converter/joy_converter_base.hpp"

namespace joy_controller
{
class G923JoyConverter : public JoyConverterBase
{
public:
  explicit G923JoyConverter(const sensor_msgs::msg::Joy & j) : j_(j) {}

  float accel() const
  {
    // Accel values go from -1 to 1
    return (AccelPedal()+1.0f)/2.0f;
  }

  float brake() const
  {
    // Brake pedals go from -1 to 1
    return BrakePedal()+1.0f;
  }
...
private:
  ...
  float AccelPedal() const { return j_.axes.at(2); } // 255 -> 0
  float BrakePedal() const { return j_.axes.at(3); } //255 -> 0 
...
#endif  // JOY_CONTROLLER__JOY_CONVERTER__G923_JOY_CONVERTER_HPP_
