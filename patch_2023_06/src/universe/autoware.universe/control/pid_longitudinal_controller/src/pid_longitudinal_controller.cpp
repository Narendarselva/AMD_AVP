...
void PidLongitudinalController::updateControlState(const ControlData & control_data)
{
  ...
  const auto changeState = [this](const auto s) {
    ...
    //NAREN
    m_control_prev_state = m_control_state;
    m_control_state = s;
    return;
  };
  ...
  const geometry_msgs::msg::Pose & current_pose, const ControlData & control_data)
{
  ...
  if (m_control_state == ControlState::DRIVE) {
    ...
    RCLCPP_INFO_THROTTLE(
      node_->get_logger(), *node_->get_clock(),3000,
      "[Stopped]. vel: %3.3f, acc: %3.3f", raw_ctrl_cmd.vel, raw_ctrl_cmd.acc);
  ...
  const double raw_acc, const ControlData & control_data)
{
  ...
  const double acc_slope_filtered =
    applySlopeCompensation(acc_max_filtered, control_data.slope_angle, control_data.shift,control_data.stop_dist);
...
double PidLongitudinalController::applySlopeCompensation(
  const double input_acc, const double pitch, const Shift shift, const double stop_dist) const
{
  // flags for state transition
  const auto & p = m_state_transition_params;
  ...
  RCLCPP_INFO_THROTTLE(node_->get_logger(),*node_->get_clock(), 1000,"pitch_orig:%lf pitch_limted:%lf sign:%lf input_acc: %lf compensated_acc: %lf  stop_dist: %lf",pitch,pitch_limited,sign,input_acc,compensated_acc,stop_dist);

  //NAREN
  //Fix for slope issue , when slope compensation is greater than the input
  //acceleration leads to negative acceleration but this will lead to stop the
  //vehicle from moving if its the beginning point in the trajectory ,
  //To over come this check the stop_dist factor and apply 0.1 standard accel
  if((input_acc > 0.1) && (compensated_acc < 0.1))
  {
    if(((m_control_prev_state == ControlState::STOPPED) || (m_control_prev_state == ControlState::STOPPING)) && 
        (m_control_state == ControlState::DRIVE))
    {
      if(stop_dist > p.drive_state_stop_dist + p.drive_state_offset_stop_dist)
      {
          RCLCPP_INFO(node_->get_logger(),"Slope compensated to 0.1 acc!!!!!!");
          return 0.1;
      }
    }
  }
