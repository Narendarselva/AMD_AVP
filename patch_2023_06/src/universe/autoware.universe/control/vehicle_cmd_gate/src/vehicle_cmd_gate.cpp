...
: Node("vehicle_cmd_gate", node_options), is_engaged_(false), updater_(this)
{
  ...
  //NAREN
  //Fix for putting the gear to parking once goal Arrived
  //Check final state of AUTO mode even if the engagement is false
  ...
  // NAREN - to put gear to park once after parking
  autoware_state_sub_ = create_subscription<autoware_auto_system_msgs::msg::AutowareState>(
    "input/state", 1, std::bind(&VehicleCmdGate::onAutowareState, this, _1));
...
void VehicleCmdGate::onAutowareState(autoware_auto_system_msgs::msg::AutowareState::ConstSharedPtr msg)
{
  autoware_state_ = msg;
}
...
void VehicleCmdGate::onTimer()
{
  ...
  if(!autoware_state_)
  {
    RCLCPP_ERROR(get_logger(),"Autoware State not available!!!"); 
    return;
  }
  // Select commands
