...
MpcLateralController::MpcLateralController(rclcpp::Node & node) : node_{&node}
{
  ...
  //NAREN - Disable path smoothing during freespace parking
  scenario_sub_ = node_->create_subscription<Scenario>(
      "/planning/scenario_planning/scenario", 1, std::bind(&MpcLateralController::onScenario, this, _1));
...
bool isParking(const Scenario::ConstSharedPtr & scenario)
{
  if (!scenario) {
    return false;
  }
...
void MpcLateralController::onScenario(const Scenario::ConstSharedPtr msg)
{
  scenario_ = msg;
}
  ...
  trajectory_follower::InputData const & input_data)
{
  ...
  if (isStoppedState()) {
    ...
    //NAREN - send steering to zero when switching from one trajectory to
    //another
    for (auto & value : m_mpc.m_input_buffer) {
      value = m_ctrl_cmd_prev.steering_tire_angle;
    }
    ...
    ctrl_cmd.steering_tire_angle = 0.0;

    output.control_cmd = ctrl_cmd;
    output.sync_data.is_steer_converged = isSteerConverged(ctrl_cmd);
    m_mpc.m_raw_steer_cmd_prev = 0.0;
    //RCLCPP_INFO(node_->get_logger(),"!!!MPC:Stopped State making steering 0.0!!!!");
    return output;
  }
...
void MpcLateralController::setTrajectory(const autoware_auto_planning_msgs::msg::Trajectory & msg)
{
  ...
  //NAREN - Disable path smoothing during freespace parking
  if(isParking(scenario_))
  {
    m_enable_path_smoothing = false;
  }
  else
  {
    m_enable_path_smoothing = true;
  }
