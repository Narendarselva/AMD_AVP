  ...
  height_system_(declare_parameter<int>("height_system", 1))
{
  ...
  //NAREN - get odom from novatel gnss driver
  odom_sub_ = create_subscription<nav_msgs::msg::Odometry>(
    "in_odom", rclcpp::QoS{1}, std::bind(&GNSSPoser::callbackOdom, this, std::placeholders::_1));
  ...
  //NAREN - publish odom to AW core modules
  odom_pub_ = create_publisher<nav_msgs::msg::Odometry>("out_odom", rclcpp::QoS{1});
  fixed_pub_ = create_publisher<tier4_debug_msgs::msg::BoolStamped>("gnss_fixed", rclcpp::QoS{1});
}

void GNSSPoser::callbackOdom(
  const nav_msgs::msg::Odometry::SharedPtr odom_msg_ptr)
{
  current_odom_msg_ = odom_msg_ptr;
}
  ...
  const sensor_msgs::msg::NavSatFix::ConstSharedPtr nav_sat_fix_msg_ptr)
{
  ...
  if(!current_odom_msg_)
  {
    RCLCPP_WARN_STREAM_THROTTLE(
      this->get_logger(), *this->get_clock(), std::chrono::milliseconds(1000).count(),
      "Yet to receive Odom . Skipping Calculate.");
     return;
  }
  ...
  //NAREN
  gnss_base_pose_msg.pose.orientation = current_odom_msg_->pose.pose.orientation; 
  ...
  gnss_base_pose_cov_msg.pose.pose.orientation =  current_odom_msg_->pose.pose.orientation;
  gnss_base_pose_cov_msg.pose.covariance[ 0] = current_odom_msg_->pose.covariance[ 0];
  gnss_base_pose_cov_msg.pose.covariance[ 7] = current_odom_msg_->pose.covariance[ 7];
  gnss_base_pose_cov_msg.pose.covariance[14] = current_odom_msg_->pose.covariance[14];
  gnss_base_pose_cov_msg.pose.covariance[21] = current_odom_msg_->pose.covariance[21];
  gnss_base_pose_cov_msg.pose.covariance[28] = current_odom_msg_->pose.covariance[28];
  gnss_base_pose_cov_msg.pose.covariance[35] = current_odom_msg_->pose.covariance[35];

  pose_cov_pub_->publish(gnss_base_pose_cov_msg);

  nav_msgs::msg::Odometry odometry{};
  odometry.header.stamp = nav_sat_fix_msg_ptr->header.stamp;
  odometry.header.frame_id = map_frame_;
  odometry.child_frame_id = "base_link";
  odometry.pose.pose.position.x = gnss_base_pose_cov_msg.pose.pose.position.x;
  odometry.pose.pose.position.y = gnss_base_pose_cov_msg.pose.pose.position.y;
  odometry.pose.pose.position.z = gnss_base_pose_cov_msg.pose.pose.position.z;
  odometry.pose.pose.orientation = gnss_base_pose_cov_msg.pose.pose.orientation;

  odometry.pose.covariance[ 0] = current_odom_msg_->pose.covariance[ 0];
  odometry.pose.covariance[ 7] = current_odom_msg_->pose.covariance[ 7];
  odometry.pose.covariance[14] = current_odom_msg_->pose.covariance[14];
  odometry.pose.covariance[21] = current_odom_msg_->pose.covariance[21];
  odometry.pose.covariance[28] = current_odom_msg_->pose.covariance[28];
  odometry.pose.covariance[35] = current_odom_msg_->pose.covariance[35];

  odometry.twist.twist.linear.x = current_odom_msg_->twist.twist.linear.x;
  odometry.twist.twist.linear.y = current_odom_msg_->twist.twist.linear.y;
  odometry.twist.twist.linear.z = current_odom_msg_->twist.twist.linear.z;
  odometry.twist.covariance[0] =  current_odom_msg_->twist.covariance[0];
  odometry.twist.covariance[7] =  current_odom_msg_->twist.covariance[7];
  odometry.twist.covariance[14] =  current_odom_msg_->twist.covariance[14];
  ...
  //Publish gnss as it is from "base_link"
  publishTF(map_frame_, base_frame_, gnss_base_pose_msg);
}
