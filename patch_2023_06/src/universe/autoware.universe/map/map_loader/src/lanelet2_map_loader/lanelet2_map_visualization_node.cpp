...
visualization_msgs::msg::MarkerArray generateParkingLotIdMarker(
  const lanelet::ConstPolygons3d  & parking_lots, const std_msgs::msg::ColorRGBA & c)
{
  visualization_msgs::msg::MarkerArray markers;
  for (const auto & pl : parking_lots) {
    if (pl.size() < 3) {
      return markers;
    }
    geometry_msgs::msg::Polygon geom_poly;
    lanelet::utils::conversion::toGeomMsgPoly(pl, &geom_poly);
 
    double x_sum = 0.0, y_sum = 0.0, z_sum = 0.0;
    for(const auto &point : geom_poly.points)
    {
      x_sum += point.x;
      y_sum += point.y;
      z_sum += point.z;
    ...
    marker.ns = "parkinglot_id";
    marker.id = static_cast<int32_t>(pl.id());
    marker.type = visualization_msgs::msg::Marker::TEXT_VIEW_FACING;
    marker.action = visualization_msgs::msg::Marker::ADD;
    marker.pose.position.x = x_sum / geom_poly.points.size() ;
    marker.pose.position.y = y_sum / geom_poly.points.size()  ;
    marker.pose.position.z = z_sum / geom_poly.points.size()  ;
    ...
    marker.text = pl.attributeOr("name", "none");
  ...
  const autoware_auto_mapping_msgs::msg::HADMapBin::ConstSharedPtr msg)
{
  ...
  std_msgs::msg::ColorRGBA cl_road, cl_shoulder, cl_cross, cl_partitions, cl_pedestrian_markings,
    ...
    cl_hatched_road_markings_line, cl_parkinglot_id;
  ...
  setColor(&cl_parkinglot_id, 0, 1, 0, 0.999);
  ...
  insertMarkerArray(
    &map_marker_array,
    generateParkingLotIdMarker(parking_lots, cl_parkinglot_id));
  insertMarkerArray(
    &map_marker_array,
