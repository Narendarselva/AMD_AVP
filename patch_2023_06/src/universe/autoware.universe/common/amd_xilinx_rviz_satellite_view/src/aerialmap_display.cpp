...
std::string const AerialMapDisplay::MAP_FRAME = "base_link";
...
void AerialMapDisplay::onInitialize()
{
  ...
  topic_property_->initialize(rviz_ros_node_);
    ...
    sub_odometry_ = rviz_ros_node_.lock()->get_raw_node()->create_subscription<nav_msgs::msg::Odometry>(
    "/localization/kinematic_state", rclcpp::QoS(1),
    std::bind(&AerialMapDisplay::onOdometry, this, std::placeholders::_1));

}

void AerialMapDisplay::onOdometry(nav_msgs::msg::Odometry::SharedPtr msg)
{
...
void AerialMapDisplay::updateCenterTile(const sensor_msgs::msg::NavSatFix::SharedPtr msg)
{
  ...
  { 
    // NAREN : Maybe we should update the transform here even if the center tile did not change?
  ...
  RVIZ_COMMON_LOG_DEBUG("Updating center tile");
...
void AerialMapDisplay::requestTileTextures()
{
  ...
  try
  {
    RVIZ_COMMON_LOG_DEBUG("Requesting new tile images from the server");
...
{
  RVIZ_COMMON_LOG_DEBUG("Starting to repaint all tiles");
  dirty_ = true;
}
...
void AerialMapDisplay::assembleScene()
{
  ...
  {
    RVIZ_COMMON_LOG_DEBUG("Finished assembling all tiles");
  }
...
void AerialMapDisplay::transformTileToMapFrame()
{
  ...
  // In assembleScene() we shift the AerialMap so that thref_fix_tile_coords.xe center tile's left-bottom corner has the coordinate (0,0).
  ...
  double const tile_w_h_m = getTileWH(ref_fix_->latitude, zoom_);
  RVIZ_COMMON_LOG_DEBUG_STREAM("Tile resolution is " << std::setprecision(1) << std::fixed << tile_w_h_m << "m");
...
void AerialMapDisplay::transformMapTileToFixedFrame()
{
  ...
  if (context_->getFrameManager()->getTransform(MAP_FRAME, t_fixed_map, o_fixed_map))
  {
    ...
    auto t_centertile_fixed = t_fixed_map + o_fixed_map * t_centertile_map_;

    //NAREN - offset added to algin with vehicle in real time
    t_centertile_fixed.x += 0.6;
    t_centertile_fixed.y -= 0.9;
