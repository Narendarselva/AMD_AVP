  ...
  vehicle_info_{vehicle_info_util::VehicleInfoUtil(node).getVehicleInfo()}
{
  ...
  if (parameters_->enable_single_arc_pull_out) {
    pull_out_planners_.push_back(std::make_shared<SingleArcPullOut>(node, *parameters));
  }
  ...
  vehicle_info_{vehicle_info_util::VehicleInfoUtil(node).getVehicleInfo()}
{
  ...
  //NAREN - Publish whether the current BH is single arc to obstacle avoidance
  //planner
  pub_is_single_arc_pull_out_ = node.create_publisher<std_msgs::msg::Bool>(
      "pull_out/is_single_arc_pull_out", rclcpp::QoS{1});
...
bool PullOutModule::isExecutionRequested() const
{
  ...
  //NAREN
  //Activate pullout module only when the ego is away from center of the lane
  const auto arc_position_ego = lanelet::utils::getArcCoordinates(current_lanes,
      planner_data_->self_odometry->pose.pose);
  if(std::fabs(arc_position_ego.distance) < 0.1)
  {
    return false;
  }
  ...
  if (LaneDepartureChecker::isOutOfLane(lanes, vehicle_footprint)) {
    ...
    is_executed_ = false;
#endif
    //NAREN Activate pullout moudle when ego is in parkinglot shoulder lane
    if(utils::isEgoInsideParkingLot(planner_data_->self_odometry->pose.pose,planner_data_->route_handler))
    {
      if(utils::isEgoInsideShoulderLane(planner_data_->self_odometry->pose.pose,planner_data_->route_handler))
      {
        return true;
      }
    }
  
    //Added this check so that we use pullout planner
    //for ego lane alignment instead of MPC control module
    //when ego base is still within the lane
    if(utils::isInLanelets(planner_data_->self_odometry->pose.pose,lanes))
    {
      return true;
    }
...
ModuleStatus PullOutModule::updateState()
{
  ...
  if (hasFinishedPullOut()) {
    //NAREN - Publish whether the current BH is single arc to obstacle avoidance
    //planner
    if(status_.is_single_arc)
    {
      std_msgs::msg::Bool msg;
      msg.data = false;
      pub_is_single_arc_pull_out_->publish(msg);
    }
...
BehaviorModuleOutput PullOutModule::plan()
{
  ...
  //NAREN - This is to skip traditional driveable area calculation when
  //we use single arc pull out in planner_manager.cpp
  output.drivable_area_info.is_single_arc = status_.is_single_arc;
  //NAREN - Publish whether the current BH is single arc to obstacle avoidance
  //planner
  if(status_.is_single_arc)
  {
  ...
  const std::vector<Pose> & start_pose_candidates, const Pose & goal_pose)
{
  ...
  for (const auto & planner : pull_out_planners_) {    
    //SINGLEARC is used only for direct start pose
    if (planner->getPlannerType() == PlannerType::SINGLEARC) {
      continue;
    }
...
void PullOutModule::planWithDirectStartPose(
  const Pose & start_pose, const Pose & goal_pose)
{
  std::cerr<<"PullOutModule::planWithDirectStartPose start"<<std::endl;
  status_.is_safe = false;
  status_.planner_type = PlannerType::NONE;
  ...
  for (const auto & planner : pull_out_planners_) {
    if (planner->getPlannerType() != PlannerType::SINGLEARC) {
      printf("Type %d %ld\n",(int) planner->getPlannerType(),pull_out_planners_.size());
      continue;
    }
    std::cerr<<"PullOutModule::planWithDirectStartPose SINGLEARC"<<std::endl;
    const auto & pull_out_start_pose = start_pose;
    planner->setPlannerData(planner_data_);
    const auto pull_out_path = planner->plan(pull_out_start_pose, goal_pose);
    // not found safe path
    if (!pull_out_path) {
      std::cerr<<"PullOutModule::planWithDirectStartPose failed"<<std::endl;
      continue;
    }
    ...
    //NAREN - This is to skip traditional driveable area calculation when
    //we use single arc pull out in planner_manager.cpp
    status_.is_single_arc = true;
    status_.pull_out_path = *pull_out_path;
    status_.pull_out_start_pose = pull_out_start_pose;
    status_.planner_type = planner->getPlannerType();
    return;
  }
  std::cerr<<"PullOutModule::planWithDirectStartPose end"<<std::endl;
}
...
void PullOutModule::updatePullOutStatus()
{
  ...
  //NAREN - To activate single arc pullout
  if((utils::isEgoInsideParkingLot(planner_data_->self_odometry->pose.pose,planner_data_->route_handler))
      && (utils::isEgoInsideShoulderLane(planner_data_->self_odometry->pose.pose,planner_data_->route_handler)))
  {
    planWithDirectStartPose(current_pose , goal_pose);
  }
  else
  {
...
bool PullOutModule::hasFinishedPullOut() const
{
  ...
  //NAREN Fix for pullpath end point close the to the ego start pose
  // For a curve path arcdistance is calculated based on cloest lanelet
  // But when ego is close tp end of pullout path in a U-shaped path
  // path arcdistance is calculated from end of pullout path
  // So check the correct cloest lanelet and then calculate the arcdis 
  lanelet::ArcCoordinates arclength_current;
  if((utils::isEgoInsideParkingLot(current_pose,planner_data_->route_handler))
      && (utils::isEgoInsideShoulderLane(current_pose,planner_data_->route_handler)))
  {
    //Get routable lanelets
    lanelet::ConstLanelets lanelet_sequence = utils::getExtendedCurrentLanesFromParkingLot(planner_data_);
    //first lanelet is the cloest lanelet from shoulder lane
    lanelet::ConstLanelet closest_lanelet = lanelet_sequence.front();
    double length = 0;
    lanelet::ArcCoordinates arc_coordinates;
    //Now use this as cloest lanelet to ego vehicle for pullout
    ...
    arclength_current = arc_coordinates;
  }
  else
  {
    arclength_current = lanelet::utils::getArcCoordinates(status_.current_lanes, current_pose);
  }
  ...
  bool has_finished = arclength_current.length - arclength_pull_out_end.length > 0.0;

  //NAREN : For SingleArc to switch to complete , check whether ego
  //is stopped  and ego reaches pull out end pose, This is to avoid sudden
  //change in trajectory when BH switches from pullout to goal planner scene
  //module
  if(status_.is_single_arc)
  {
    const double ego_vel = utils::l2Norm(planner_data_->self_odometry->twist.twist.linear);
    has_finished = (arclength_current.length - arclength_pull_out_end.length > -0.5) && (ego_vel < 0.1);
    RCLCPP_INFO_THROTTLE(
        getLogger(), *clock_, 2000, "arclength_current:%lf arclength_pull_out_end:%lf diff:%lf has_finished:%d ego_vel:%lf",arclength_current.length,arclength_pull_out_end.length,arclength_current.length - arclength_pull_out_end.length,has_finished,ego_vel);
