...
//NAREN - TO check ego inside a parking lot
bool isEgoInsideParkingLot(const Pose & self_pose,const std::shared_ptr<RouteHandler> & route_handler)
{
  //check ego vehicle is inside a parking lot
  const auto all_parking_lots = route_handler->getParkingLots();
  const bool is_in_parking_lot = std::invoke([&]() {
      const auto & p = self_pose.position;
      ...
      lanelet::utils::query::getLinkedParkingLot(
          search_point.basicPoint2d(), all_parking_lots, nearest_parking_lot.get());
  ...
  return is_in_parking_lot;
}

//NAREN - To check Ego inside a shoulder lane
bool isEgoInsideShoulderLane(const Pose & self_pose,const std::shared_ptr<RouteHandler> & route_handler)
{
  const auto shoulder_lanes = route_handler->getShoulderLanelets();

  if (utils::isInLanelets(self_pose, shoulder_lanes)) {
    return true;
  }
  return false;
}
  ...
  const std::shared_ptr<RouteHandler> & route_handler)
{
  ...
  if (!is_in_shoulder_lane && !is_in_road_lane && !is_in_parking_lot) {
    return true;
  }
  ...
  for (size_t i = 0; i < path.points.size(); ++i) {
    ...
    for (const auto & lane_id : lane_ids) {
      if (
        ...
        //TO CHECK NAREN: Temp fix for routes end lane connects to
        //start of the route (looping route) . Crash issue when 
        //utils::generateDrivableArea since below line erases all the path
        //points
...
//NAREN - Generate drivable area when doing summon using single arc algo
void generateDrivableAreaSingleArcPullout(
  PathWithLaneId & path, const std::vector<DrivableLanes> & lanes,
  ...
  const std::shared_ptr<const PlannerData> planner_data, const bool is_driving_forward)
{
  ...
  const auto [left_start_idx, right_start_idx] = [&]() {
    ...
    const auto cropped_path_points = motion_utils::cropPoints(
      ...
      planner_data->parameters.forward_path_length, 0.5 );
...
lanelet::ConstLanelets getCurrentLanes(const std::shared_ptr<const PlannerData> & planner_data)
{
  ...
  //NAREN for summon scenario which has U-turn like route
  //the cloest lane is not properly calculated.Get the cloest
  //lanelet within the parking lot
  if(isEgoInParkingLot(planner_data))
  {
    if(isEgoInsideShoulderLane(current_pose,route_handler))
    {
      return getExtendedCurrentLanesFromParkingLot(planner_data);
    }
  }
...
//NAREN 
bool isEgoInParkingLot(
    const std::shared_ptr<const PlannerData> & planner_data)
{
  const auto & route_handler = planner_data->route_handler;
  const auto current_pose = planner_data->self_odometry->pose.pose;
  if(const auto nearest_parking_lot = route_handler->getParkingLot(current_pose))
  {
    return true;
  }
  else
...
//NAREN Get lanelet seq for summon scenario
lanelet::ConstLanelets getExtendedCurrentLanesFromParkingLot(
    const std::shared_ptr<const PlannerData> & planner_data)
{
  const auto & route_handler = planner_data->route_handler;
  const auto current_pose = planner_data->self_odometry->pose.pose;
  ...
  if(const auto nearest_parking_lot = route_handler->getParkingLot(current_pose))
  {
    //Get parking lot linked lanelets
    lanelet::ConstLanelets candidate_lanelets = route_handler->getLinkedLanelets(*nearest_parking_lot);
    //Cloest routable lanelet from the parkinglot linked lanelets
    for (const auto & llt : candidate_lanelets)
    {
      //See if lanelet is present in the routeable lanelets
      lanelet::ConstLanelets current_lanelets = route_handler->getLaneletSequence(
          llt, current_pose, common_parameters.backward_path_length,
          common_parameters.forward_path_length);
  ...
  std::cerr << "failed to find closest lanelet within parkinglot!!!" << std::endl;
  return {};
}
