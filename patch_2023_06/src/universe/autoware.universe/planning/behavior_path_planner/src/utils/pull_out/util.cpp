...
lanelet::ConstLanelets getPullOutLanes(const std::shared_ptr<const PlannerData> & planner_data)
{
  ...
  //NAREN for summon scenario get the proper shoulder lane
  //which can be overlapped as pullout lane
  if(utils::isEgoInParkingLot(planner_data))
  {
    const auto & shoulder_lanelets = route_handler->getShoulderLanelets();
    //Get the routeable lanelets that is used to reach the goal pose
    lanelet::ConstLanelets lanelet_seq = utils::getExtendedCurrentLanesFromParkingLot(planner_data);
    //First lane is the cloest lane which is connected the shoulder lane to
    //pullout the ego
    lanelet::ConstLanelet current_lane = lanelet_seq.front();
    for (const auto & shoulder_lanelet : shoulder_lanelets)
    {
      if (lanelet::utils::isInLanelet(current_pose, shoulder_lanelet, vehicle_width / 2.0))
      {
        if ((shoulder_lanelet.leftBound().id() == current_lane.rightBound().id())
            || (shoulder_lanelet.rightBound().id() == current_lane.leftBound().id()))
        {
          std::cerr<<"Selected Shoulder lane:"<<shoulder_lanelet.id()<<std::endl;
          return route_handler->getShoulderLaneletSequence(shoulder_lanelet, current_pose);
        }
      }
    }
  }
