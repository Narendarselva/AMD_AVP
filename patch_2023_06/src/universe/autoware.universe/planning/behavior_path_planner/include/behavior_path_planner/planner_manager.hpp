...
private:
  ...
  BehaviorModuleOutput getReferencePath(const std::shared_ptr<PlannerData> & data) const
  {
    ...
    //NAREN - Properly updating ref path based on cloest lane when the ego is an shoulder lane
    //parking lot in case of summon scenario
    if(utils::isEgoInParkingLot(data))
    {
      if(utils::isEgoInsideShoulderLane(pose,route_handler))
      {
        lanelet::ConstLanelets routable_lanelets = utils::getExtendedCurrentLanesFromParkingLot(data);
        if(!routable_lanelets.empty())
        {
          lanelet::ConstLanelet closest_lane = routable_lanelets.front();
  ...
  lanelet::ConstLanelet updateRootLanelet(const std::shared_ptr<PlannerData> & data) const
  {
    ...
    //NAREN - Properly updating the root lane(cloest lane) when the ego is an shoulder lane
    //parking lot in case of summon scenario
    if(utils::isEgoInParkingLot(data))
    {
      lanelet::ConstLanelets routable_lanelets = utils::getExtendedCurrentLanesFromParkingLot(data);
