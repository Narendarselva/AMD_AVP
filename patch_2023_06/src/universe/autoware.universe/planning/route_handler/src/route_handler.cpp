...
void RouteHandler::setMap(const HADMapBin & map_msg)
{
  ...
  all_parking_lots_ = lanelet::utils::query::getAllParkingLots(lanelet_map_ptr_);
...
lanelet::ConstPolygons3d RouteHandler::getParkingLots() const
{
  return all_parking_lots_;
}
...
std::shared_ptr<lanelet::ConstPolygon3d> RouteHandler::getParkingLot(
    const geometry_msgs::msg::Pose & current_pose) const
{
  const auto & p = current_pose.position;
  const lanelet::Point3d search_point(lanelet::InvalId, p.x, p.y, p.z);

  const auto nearest_parking_lot = std::make_shared<lanelet::ConstPolygon3d>();
  lanelet::utils::query::getLinkedParkingLot(
      search_point.basicPoint2d(), all_parking_lots_, nearest_parking_lot.get());
  if (!nearest_parking_lot) {
    return {};
  }
  if(lanelet::geometry::within(search_point, nearest_parking_lot->basicPolygon()))
  {
...
lanelet::ConstLanelets RouteHandler::getLinkedLanelets(
  const lanelet::ConstPolygon3d & parking_lot)
{
  return lanelet::utils::query::getLinkedLanelets(parking_lot,road_lanelets_); 
}

bool RouteHandler::getClosestLaneletFromPose(const geometry_msgs::msg::Pose & current_pose,
    lanelet::Lanelet *closest_lanelet)
{
  return lanelet::utils::query::getClosestLanelet(road_lanelets_, current_pose ,closest_lanelet);
}
  ...
  lanelet::ConstLanelets * path_lanelets) const
{
  ...
  const lanelet::ConstLanelet *final_start_llt = nullptr;
  ...
  if(const auto nearest_parking_lot = getParkingLot(goal_checkpoint); nearest_parking_lot)
  {
    //Parking Scenario
    final_start_llt = &start_lanelet;
    final_goal_llt = &goal_lanelet;
    //This is to check the shortest path which can be used in case of
    //overlapping lanes inside parking lot
    RCLCPP_INFO(logger_,"Parking Scenario!!!");
    candidate_lanelets = lanelet::utils::query::getLinkedLanelets(*nearest_parking_lot,road_lanelets_);
    for (const auto & llt : candidate_lanelets)
    {
      RCLCPP_INFO_STREAM(
          logger_, "Lanelets inside parkingLot: " << llt.id()<<std::endl
          );
      lanelet::Optional<lanelet::routing::Route> optional_route = routing_graph_ptr_->getRoute(start_lanelet, llt, 0);
      if(optional_route)
      {
        int path_distance = 0;
        ...
        RCLCPP_INFO_STREAM(
            logger_, "path distance from lanelet:" << llt.id() <<" -> "
  ...
  else if(const auto nearest_parking_lot = getParkingLot(start_checkpoint); nearest_parking_lot) 
  {
    RCLCPP_INFO(logger_,"Summon Scenario!!!");
    //Summon Scenario
    final_start_llt = &start_lanelet;
    final_goal_llt = &goal_lanelet;
    lanelet::ConstLanelet ego_shoulder_lane;
    ...
    for (const auto & llt : candidate_lanelets)
    {
      ...
      lanelet::Optional<lanelet::routing::Route> optional_route = routing_graph_ptr_->getRoute(llt, goal_lanelet, 0);
      if(optional_route)
      {
        for (const auto & shoulder_lanelet : shoulder_lanelets_)
        {
          if (lanelet::utils::isInLanelet(start_checkpoint, shoulder_lanelet, 1.0))
          {
            ego_shoulder_lane = shoulder_lanelet;
            if ((ego_shoulder_lane.leftBound().id() == llt.rightBound().id())
                || (ego_shoulder_lane.rightBound().id() == llt.leftBound().id()))
            {
              int path_distance = 0;
              const auto shortest_path = optional_route->shortestPath();
              for (const auto & llt : shortest_path)
  ...
  {
    //Default Scenario APA
    final_start_llt = &start_lanelet;
    final_goal_llt = &goal_lanelet;
  }

  if((final_start_llt) && (final_goal_llt))
  {
    RCLCPP_INFO_STREAM(
        logger_,"!!!Found proper path!!!"<<std::endl
        <<"start checkpoint: " << toString(start_checkpoint) << std::endl
        << "goal checkpoint: " << toString(goal_checkpoint) << std::endl
        << "final_start lane id: " << final_start_llt->id() << std::endl
        << "final_goal lane id: " << final_goal_llt->id() << std::endl
        );

    lanelet::Optional<lanelet::routing::Route> optional_route = routing_graph_ptr_->getRoute(*final_start_llt, *final_goal_llt, 0);
    if (!optional_route)
    {
      RCLCPP_ERROR_STREAM(
