...
using motovis_interface_msgs::srv::UpdateGoalPose;
...
private:
  ...
  rclcpp::Publisher<PoseArray>::SharedPtr checkpoint_array_pub_;
  ...
  rclcpp::Client<UpdateGoalPose>::SharedPtr update_goal_pose_cli_;
  ...
  double vehicle_max_steer_angle_;
  ...
  bool isParallelParkingInProgress_ = false;
  bool ignoreObstacleReplan_ = false;
  std::mutex route_update_mtx;
  ...
  //Parallel parking
  std::vector<Trajectory> generatePullOverPaths(
  ...
  std::vector<Trajectory> planOneTrialRight(
  const Pose & start_pose, const Pose & goal_pose, const double R_E_r,
  const bool is_forward, const double end_pose_offset,
  const double arc_path_interval);
  std::vector<Trajectory> planOneTrialLeft(
  const Pose & start_pose, const Pose & goal_pose, const double R_E_r,
  const bool is_forward, const double end_pose_offset,
  const double arc_path_interval);
  Trajectory generateArcPath(
  ...
  TrajectoryPoint generateArcPathPoint(
