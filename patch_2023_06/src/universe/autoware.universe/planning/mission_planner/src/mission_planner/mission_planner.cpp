  ...
  normal_route_(nullptr)
{
  ...
  declare_parameter<bool>("is_avp_scenario",is_avp_scenario_);
  get_parameter("is_avp_scenario",is_avp_scenario_);
  std::cout << "is_avp_scenario: " << is_avp_scenario_ << std::endl;
...
void MissionPlanner::on_clear_route(
  const ClearRoute::Service::Request::SharedPtr req, const ClearRoute::Service::Response::SharedPtr res)
{
  clear_route();
  change_state(RouteState::Message::UNSET);
  res->status.success = true;

  //This is triggered from avp_planner module
  if(req->is_avp_triggered)
  {
    is_avp_triggered_ = req->is_avp_triggered;
    RCLCPP_INFO(get_logger(),"is_avp_triggered from avp_planner module switching from LaneDriving -> Parking mode");
  }
}
  ...
  const SetRoutePoints::Service::Response::SharedPtr res)
{
  ...
  {
    //AVP
    RCLCPP_INFO(get_logger(),"MissionPlanner::on_set_route_points AVP called");
    if (!planner_->ready()) {
      throw component_interface_utils::ServiceException(
          ResponseCode::ERROR_PLANNER_UNREADY, "The planner is not ready.");
    }
    
    //When switching from lanedriving to parking scenario
    //we never need to check the create_route(req) function
    //as the final route is only needed by the freespace planner not the BH
    //planner
    //This is added to fix the issue when ego switches from LaneDriving to
    //Parking when create_route fails some time at the edge of the parking lot
    //as one of the wayepoint is out side of the parking lot
    if(!is_avp_triggered_)
    {
      // Plan route.
    ...
    else
    {
      ...
      route.uuid.uuid = generate_random_id();
      route.start_pose = odometry_->pose.pose;
      route.goal_pose = req->goal;
      is_avp_triggered_ = false;
    }
  ...
  {
    RCLCPP_INFO(get_logger(),"MissionPlanner::on_set_route_points APA called");
    //APA NAREN TODO
    // Use temporary pose stamped for transform.
    PoseStamped pose;
    pose.header = req->header;
