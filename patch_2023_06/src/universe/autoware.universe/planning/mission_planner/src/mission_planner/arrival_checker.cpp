...
ArrivalChecker::ArrivalChecker(rclcpp::Node * node) : vehicle_stop_checker_(node),node_(node)
{
...
{
  if (!goal_with_uuid_) {
	  RCLCPP_INFO_THROTTLE(node_->get_logger(), *(node_->get_clock()), 1000, "ArrivalChecker: no uuid");
    return false;
  }
  ...
  if (goal.header.frame_id != pose.header.frame_id) {
	   RCLCPP_INFO_THROTTLE(node_->get_logger(), *(node_->get_clock()), 1000, "ArrivalChecker: Frame id different");
    return false;
  }

  // Check distance.
  if (distance_ < tier4_autoware_utils::calcDistance2d(pose.pose, goal.pose)) {
    RCLCPP_INFO_THROTTLE(node_->get_logger(), *(node_->get_clock()), 5000, "ArrivalChecker: larger distance goal %f < %f",
        distance_,tier4_autoware_utils::calcDistance2d(pose.pose, goal.pose));
    return false;
  }
  ...
  if (angle_ < std::fabs(yaw_diff)) {
	  RCLCPP_INFO_THROTTLE(node_->get_logger(), *(node_->get_clock()), 5000, "ArrivalChecker: Larger angle diff %f",tier4_autoware_utils::rad2deg(yaw_diff));
    return false;
  }
