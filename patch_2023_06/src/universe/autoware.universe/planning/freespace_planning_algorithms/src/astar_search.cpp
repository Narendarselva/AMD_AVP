...
void AstarSearch::stopPlanning()
{
  //if already running make it stop
  if(isPlanningInProgress_)
  {
    std::cerr<<"FreespaceAlgo: Stopped Planning!!!"<<std::endl;
    isStopCondition_ = true;
  }
}
  ...
  const geometry_msgs::msg::Pose & start_pose, const geometry_msgs::msg::Pose & goal_pose)
{
  ...
  if (!setStartNode()) {
    std::cerr<<"FreespaceAlgo: Cant set Start node!!!"<<std::endl;
    return false;
  }

  if (!setGoalNode()) {
    std::cerr<<"FreespaceAlgo: Cant set Goal node!!!"<<std::endl;
    return false;
  }
...
bool AstarSearch::search()
{
  ...
  while (!openlist_.empty()) {
    ...
    if (msec > planner_common_param_.time_limit) {
      std::cerr<<"FreespaceAlgo:Failed to find the path, TimedOut"<<std::endl;
      isStopCondition_ = false;
      isPlanningInProgress_ = false;
      return false;
    }
    ...
    if(isStopCondition_)
    {
      isStopCondition_ = false;
      isPlanningInProgress_ = false;
      std::cerr<<"FreespaceAlgo:Failed to find the path, StoppedCondition new goal"<<std::endl;
      return false;
    }
    ...
    if (isGoal(*current_node)) {
      ...
      isStopCondition_ = false;
      isPlanningInProgress_ = false;
      std::cerr<<"FreespaceAlgo:Success!!!"<<std::endl;
      return true;
    }
    ...
    for (const auto & transition : transition_table_[index_theta]) {
      ...
      //NAREN - To elminate short trajectory creations 
      if(is_turning_point)
      {
        if(current_node->partial_traj > 1)
        {
          continue;
        }
      }
      ...
      if (next_node->status == NodeStatus::None) {
        ...
        next_node->partial_traj = current_node->partial_traj;
        if(is_turning_point)
        {
          next_node->partial_traj++;
        }
  ...
  std::cerr<<"FreespaceAlgo:Failed to find the path"<<std::endl;
  // Failed to find path
  isStopCondition_ = false;
  isPlanningInProgress_ = false;
  return false;
