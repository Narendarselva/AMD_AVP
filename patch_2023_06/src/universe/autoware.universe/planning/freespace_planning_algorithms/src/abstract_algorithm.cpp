...
//NAREN
void AbstractPlanningAlgorithm::resetObstacleTable()
{
  is_obstacle_table_ = dummy_obstacle_table_;
}
...
void AbstractPlanningAlgorithm::setMap(const nav_msgs::msg::OccupancyGrid & costmap)
{
  ...
  //NAREN construct collision indexes table && dummy obstacle table with no
  //obstacles
  if (is_collision_table_initialized == false) {
    ...
    //dummy obstacle table
    dummy_obstacle_table_.resize(height);
    for (uint32_t i = 0; i < height; i++)
    {
      dummy_obstacle_table_.at(i).resize(width);
      for (uint32_t j = 0; j < width; j++)
      {
        dummy_obstacle_table_[i][j] = false;
      }
    }
...
//NAREN
size_t AbstractPlanningAlgorithm::getObstacleIndexOnTrajectory(
  const geometry_msgs::msg::PoseArray & trajectory) const
{
  for (size_t i=0; i < trajectory.poses.size(); i++) {
    const auto pose_local = global2local(costmap_, trajectory.poses[i]);
    const auto index = pose2index(costmap_, pose_local, planner_common_param_.theta_size);
