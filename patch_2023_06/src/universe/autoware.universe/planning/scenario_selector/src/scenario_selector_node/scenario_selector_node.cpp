...
bool isInLanelets(const geometry_msgs::msg::Pose & pose, const lanelet::ConstLanelets & lanes)
{
...
bool isInShoulderLane(const std::shared_ptr<route_handler::RouteHandler> & route_handler,
    const geometry_msgs::msg::Pose & self_pose)
{
  const auto shoulder_lanes = route_handler->getShoulderLanelets();

  if (isInLanelets(self_pose, shoulder_lanes)) {
    return true;
  }
  return false;
}
...
{
  const auto is_ego_in_lane = isInLane(lanelet_map_ptr_, current_pose_->pose.pose.position);
  const auto is_goal_in_lane = isInLane(lanelet_map_ptr_, route_->goal_pose.position);
  const auto is_ego_in_parking_lot = isInParkingLot(lanelet_map_ptr_, current_pose_->pose.pose);
  const auto is_goal_in_parking_lot = isInParkingLot(lanelet_map_ptr_, route_->goal_pose);
  const auto is_ego_in_shoulder_lane = isInShoulderLane(route_handler_, current_pose_->pose.pose);
  const auto is_goal_in_shoulder_lane = isInShoulderLane(route_handler_, route_->goal_pose);

  if (current_scenario_ == tier4_planning_msgs::msg::Scenario::EMPTY) {
    if (is_ego_in_lane && (is_goal_in_lane && !is_goal_in_shoulder_lane)) {
      return tier4_planning_msgs::msg::Scenario::LANEDRIVING;
    } else if ((is_ego_in_parking_lot && is_ego_in_lane) &&
        (is_goal_in_shoulder_lane && is_goal_in_parking_lot)) {
  ...
  if (current_scenario_ == tier4_planning_msgs::msg::Scenario::LANEDRIVING) {
    if ((is_ego_in_parking_lot && is_ego_in_lane) &&
        (is_goal_in_shoulder_lane && is_goal_in_parking_lot)) {
      return tier4_planning_msgs::msg::Scenario::PARKING;
    }
  }
  ...
  if (current_scenario_ == tier4_planning_msgs::msg::Scenario::PARKING) {
    if ((is_ego_in_parking_lot && is_ego_in_shoulder_lane) &&
        (is_goal_in_lane && !is_goal_in_shoulder_lane))
    {
      return tier4_planning_msgs::msg::Scenario::LANEDRIVING;
    }
  }
