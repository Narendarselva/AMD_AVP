  ...
  time_keeper_ptr_(std::make_shared<TimeKeeper>())
{
  ...
  //NAREN - Skip optimization if its a single arc pullout
  is_single_arc_pull_out_sub_ = create_subscription<std_msgs::msg::Bool>(
    "/planning/scenario_planning/lane_driving/behavior_planning/pull_out/is_single_arc_pull_out", 1, [this](const std_msgs::msg::Bool::ConstSharedPtr msg) { is_single_arc_pull_out_ = msg->data; });
  ...
  const PlannerData & planner_data)
{
  ...
  //NAREN - Skip optimization if its a single arc pullout
  if (enable_skip_optimization_ || is_single_arc_pull_out_) {
  ...
  const PlannerData & planner_data, std::vector<TrajectoryPoint> & optimized_traj_points) const
{
  ...
  //NAREN - Skip optimization if its a single arc pullout
  if (!enable_outside_drivable_area_stop_ || is_single_arc_pull_out_) {
    return;
  }
