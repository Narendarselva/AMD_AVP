  ...
  vehicle_info_(vehicle_info_util::VehicleInfoUtil(*this).getVehicleInfo())
{
  ...
  /* parameter for using tire angle for HIL setup */
  use_steering_tire_angle_directly_ = declare_parameter("use_steering_tire_angle_directly", false);
  ...
  turn_rpt_sub_ = create_subscription<pacmod3_msgs::msg::SystemRptInt>(
    "/pacmod/turn_rpt", 10, std::bind(&PacmodInterface::onTurnRpt, this, _1));
  ...
  pacmod_feedbacks_sync_ =
    std::make_unique<message_filters::Synchronizer<PacmodFeedbacksSyncPolicy>>(
      ...
      *brake_rpt_sub_, *shift_rpt_sub_, *global_rpt_sub_); // NAREN
...
void PacmodInterface::onTurnRpt(const pacmod3_msgs::msg::SystemRptInt::ConstSharedPtr msg)
{
  turn_rpt_ptr_ = msg;
}
  ...
  const ControlModeCommand::Response::SharedPtr response)
{
  ...
  RCLCPP_INFO(get_logger(),"!!!!Received request to change to AUTONOMOUS Mode!!!!");
    engage_cmd_ = true;
  ...
  RCLCPP_INFO(get_logger(),"!!!!Received request to change to MANUAL Mode!!!!");
    engage_cmd_ = false;
    is_clear_override_needed_ = true;
    response->success = true;
    return;
...
void PacmodInterface::callbackPacmodRpt(
  ...
  const pacmod3_msgs::msg::GlobalRpt::ConstSharedPtr global_rpt) // NAREN
{
  ...
  double current_steer = current_steer_wheel / adaptive_gear_ratio + steering_offset_;

  //NAREN: To use directly the tire angle for cognata HIL setup
  if(use_steering_tire_angle_directly_)
  {
    current_steer = current_steer_wheel;
  }
...
void PacmodInterface::publishCommands()
{
  ...
  /* publish steering cmd */
  {
    ...
    if(use_steering_tire_angle_directly_)
    {
      steer_cmd.command = control_cmd_ptr_->lateral.steering_tire_angle;
    }
  ...
  if (turn_indicators_cmd_ptr_ && hazard_lights_cmd_ptr_ && turn_rpt_ptr_) {
    /* publish shift cmd */
